    ...
    // Displays total price of products in cart
        ...
        private void handleGenerateCartFacture() {
            ...
            double total = 0;
            StringBuilder details = new StringBuilder();// Calculate total based on current quantities
            for (CartItem item : panier) {
        ...
        private void generateFactureForCartItem(CartItem item) {
            ...
            String details = item.getProduit().getNom() + ": " + item.getQuantity();// Create the Facture object
            Facture facture = new Facture(item.getProduit().getId(), total, details);// Add the facture to the database
            FactureService factureService = new FactureService();
            if (factureService.addFacture(facture)) {// Generate the PDF
                FacturePDFGenerator.generatePDF(facture);
            }
        ...
        private VBox createProductCard(Produit produit) {
            ...
            priceLabel.setStyle("-fx-text-fill: #27ae60; -fx-font-size: 14px; -fx-font-weight: bold;");// Existing feedback display (if any)
            ...
            feedbackLabel.setStyle("-fx-font-size: 12px; -fx-text-fill: #555555;");// "Feedback" button
            ...
            btnFeedback.setOnAction(e -> handleFeedback(produit));// "Purchase" button
            ...
            HBox actionButtons = new HBox(10, btnFeedback, btnPurchase);// Assemble card
        ...
        public void handleConfirmPurchase() {
            ...
            int userId = userSession.getUser().getId();  // Get the logged-in user ID from the session// Register the order (commande) using CommandeService and associate it with the user
            ...
            FactureService factureService = new FactureService();
            if (factureService.addFacture(facture)) {// Generate the PDF for the facture
                FacturePDFGenerator.generatePDF(facture);
                showAlert("Success", "Your order has been registered and facture generated.");// Clear the cart and update the total
                panier.clear();

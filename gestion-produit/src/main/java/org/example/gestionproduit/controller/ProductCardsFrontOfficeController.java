...
// Displays total price of products in cart
    ...
    public void handleConfirmPurchase() {
        ...
        dialog.showAndWait().ifPresent(result -> {
            ...
            String etatPaiement = paiementType.equals("en ligne") ? "payée" : "non payée";

            Runnable createCommande = () -> {
                Commande commande = new Commande(
                        0, total, productsList.toString(), new Timestamp(System.currentTimeMillis()),
                        user.getId(), selectedAddress, paiementType,
                        paiementType.equals("en ligne") ? "validée" : "en cours", user.getNom()
                );
                int commandeId = commandeService.addCommandeAndReturnId(commande);
                if (commandeId == -1) {
                    Platform.runLater(() -> showAlert("Erreur", "Échec commande."));
                    return;
                }
                ...
                if (new FactureService().addFacture(facture)) {
                    FacturePDFGenerator.generatePDFWithSignature(facture);
                    sendSlackNotification(user.getNom(), commandeId, total, paiementType, etatPaiement, selectedAddress);
                    Platform.runLater(() -> {
                        showAlert("Succès", "Commande confirmée et facture générée.");
                        finalizeCommande(commande);
                    });
                } else {
                    Platform.runLater(() -> showAlert("Erreur", "Échec de la facture."));
                }
            };
                ...
                try {
                    ...
                    stripeEngine.locationProperty().addListener((obs, oldLoc, newLoc) -> {
                        if (newLoc.contains("https://example.com/success")) {
                            ...
                            stripeStage.close();
                            Platform.runLater(() -> showAlert("Paiement annulé", "Le paiement a été annulé."));
                        }
                    });
    ...
    private void sendSlackNotification(String userNom, int commandeId, double total, String paiementType, String etatPaiement, String address) {
        try {
            String webhookUrl = "https://hooks.slack.com/services/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";
            String message = String.format(
                    "📦 Nouvelle commande !\n👤 Client : %s\n🆔 Commande ID : %d\n💰 Total : %.2f €\n💳 Paiement : %s (%s)\n📍 Adresse : %s",
                    userNom, commandeId, total, paiementType, etatPaiement, address
            );
            ...
            client.sendAsync(request, java.net.http.HttpResponse.BodyHandlers.ofString())
                    .thenAccept(response -> System.out.println("✅ Slack message sent: " + response.statusCode()));
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
